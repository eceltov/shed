...
const FileStructureDocument = require('./FileStructureDocument');
...
class FileStructureFolder extends React.Component {
  ...
  shouldBeChecked() {
    return this.state.checked
      || this.props.toSpawnDocument === this.props.fileID
      || this.props.toSpawnFolder === this.props.fileID;
  }
    ...
    return (
      <li>
        ...
        <ul className={this.getULClassName()}>
          {this.props.items === null
            ? null
            : Object.entries(this.props.items).map((keyValuePair) => (
              this.createItem(keyValuePair[1], keyValuePair[0])))}
        </ul>
      </li>
    );
  }
  ...
  createItem(fileObj) {
    ...
    if (fileObj.type === fsOps.types.folder) {
      return this.createFolder(
        fileObj.ID, fileObj.name, fileObj.items, fileObj.ID === this.props.toBeRenamed,
      );
    }
  ...
  render() {
    return this.props.renamable
      ? this.createRenamableFolderContent()
